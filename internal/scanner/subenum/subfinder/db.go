...
func PushSubdomainsToDB(dbClient *db.DB, domainID int, subdomains []string) error {
	if len(subdomains) == 0 {
		return nil
	}
	if err := dbClient.ExecStmt(SQL_CREATE_SUBDOMAINS_TABLE); err != nil {
		return fmt.Errorf("failed to create subdomains table: %v", err)
	}
